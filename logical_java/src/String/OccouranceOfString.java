...
//		 words.stream().collect(Collectors.groupingBy(word-> word ,Collectors.counting())).forEach((a,b)-> System.out.println(a+" :"+b));
		 words.stream().collect(Collectors.groupingBy(String :: length)).forEach((c,d)->System.out.println(c+" : "+d));
		 
			List<Integer> number = Arrays.asList(1,2,3,4,5,657);
			number.stream().sorted(Collections.reverseOrder()).distinct().limit(4).skip(3l).forEach( num -> System.out.print(num));
